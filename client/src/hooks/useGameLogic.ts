...
> = {
  wheat: { emoji: '🌾', name: 'Wheat', growTime: 3, cost: 0, sellPrice: 10 },
  corn: { emoji: '🌽', name: 'Corn', growTime: 5, cost: 50, sellPrice: 80 },
  gold: { emoji: '🌟', name: 'Gold', growTime: 8, cost: 150, sellPrice: 300 },
}
...
export function useGameLogic() {
  ...
  const plant = useCallback(
    (cropRaw: string) => {
      ...
      const crop = cropRaw as CropKey
      const spec = CROPS[crop]
      if (!spec) {
        addLog('❌ Unknown crop. Use: wheat, corn, gold', 'error')
        return
      }
